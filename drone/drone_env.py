...
#print(f"Actual vel (world) : mean={actual_vel_w.mean(dim=0)}")
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        obstacle_detected = min_depth < 0.25 #we might need to tune the 0.15
        
        if self.common_step_counter % 500 == 0:
            print(f"min_depth mean={min_depth.mean():.3f}, median={min_depth.median():.3f}, min={min_depth.min():.3f}, max={min_depth.max():.3f}")
